		...
		<div class="row bg-primary_bg lg:mb-4 pt-20">
			<div class="container grid grid-cols-1 lg:grid-cols-8 lg:gap-10 items-center mx-auto p-4">
				<div class="col-span-full lg:col-span-6">
					<p class="gamesSectionHead text-center lg:text-left text-3xl text-primary py-4 px-4">{{ msgTranslate?.scratchcard_games || 'Scratchcard Games' }}</p>
					<div v-for="promo in promotionsPosts" :key="promo.id">
						<div class="info_content text-primary font-extralight py-5 px-4">{{ promo.acf.scratchcard_games_info }}</div>
					</div>
				</div>
...
useHead({
	title: 'Scratchcards - Dukes Casino',
	meta: [
		{ hid: 'description', name: 'description', content: 'Explore the best scratchcards games available at Dukes Casino!' },
		{ name: 'keywords', content: 'scratchcards, games, casino, Dukes Casino' }
	]
});
</script>
